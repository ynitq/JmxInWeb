package com.fireNut.jmxInWeb.utils;
...
public class BinderEditorSupport {
	...
	private static final org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory.getLog(BinderEditorSupport.class);
	...
	public static void updateObj(Map<String, List<String>> map, Object form) {
				...
				if (paramTypes.length == 1 && instance.isValidType(paramTypes[0])) {
					...
					if (values != null && values.size() > 0) {
						...
						if (log.isDebugEnabled()) {
							...
							log.debug(LogUtil.format("需要设置的属性:%s, 类型:%s, 值:%s", propName, param.getSimpleName(),
						...
						if (param.isArray()) {
							// 如果要设置的是数组
							propValue = Array.newInstance(param.getComponentType(), values.size());
							for (int i = 0; i < values.size(); i++) {
								Object arg = instance.getValueFormString(propName, values.get(i), param.getComponentType());
								Array.set(propValue, i, arg);
							}
						} else {
							propValue = instance.getValueFormString(propName, values.get(0), param);
						}
						try {
